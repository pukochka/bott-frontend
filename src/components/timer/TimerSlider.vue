...
import { setCaretPosition } from 'src/utils/helpers/selection';
...
const setTime = () => {
  let correctMap = getFormattedTime(props.value);
  ...
  Object.entries(setting.value).forEach(([_, value], index) => {
    ...
    if (time === false) {
      ...
      const num = Number(time?.slice(0, 2) ?? 0);
