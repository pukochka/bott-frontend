...
import { defaultQuiz } from '../../stores/FBModels';
...
const deleteQuizItem = () => {
  ...
  fetchFeedback('delete-select-option', { option_id: props.item.id }).then(
    () => {
...
const moveQuiz = (side: 'up' | 'down') => {
  ...
  fetchFeedback(`select-option-${side}`, { option_id: props.item.id }).then(
    () => {
